...
Now we can run the BSure algorithm
...
2. proportion_core_essential_in_essential_I, the proportion genes of essentiality type I among listed core essential and core fitness genes should be high, with a recommended minimum of 0.75.
...
Now we check how well genes of different essentiality types are retained with the lower Cas9 and coverage levels, and only 3' readout (but highest experiment coverage):
...
<em>proportion_genes_recovered </em>is the proportion of genes from the gold standard set that are also contained in the corresponding set for the new screen, corrected for the false positive rate for the gold standard screen. The closer to one, the better the two screens agree in identifying genes at different essentiality types. 

<em>proportion_genes_not_in_gold_standard_set</em> is the proportion of genes in the new set that is not in the gold standard set, corrected for false negative rate. The closer to 0, the better. 

This is a negative example to show that we cannot recapitulate the same sets of different levels of essential genes if we both allow for lower Cas9 levels and replace the combination of 3' and 5' readout used in the double-gRNA screens in @Peets2019 by 3' readout only. 
...
# Comparing across cell lines

To compare essentiality types in different cell lines (here K562 from @Peets2020 and HT29 from @Allen2019) robustly, we
compare sets of essential genes of levels I and II (with significance levels as detailed above) and sets of unessential genes of levels I and II (unessential with the same significance levels), and determine if there are overlaps between significantly essential genes in one cell line and significantly unessential genes in the other. 

```{r}
intersect(extracted_gene_categories_K562_best$not_essential_genes_II,extracted_gene_categories_HT29$essential_genes_II)
...
```
```{r}
intersect(extracted_gene_categories_K562_best$essential_genes_I,extracted_gene_categories_HT29$not_essential_genes_I)
extracted_gene_categories_K562_best$false_positive_rate_essential_I
extracted_gene_categories_HT29$false_positive_rate_not_essential_I
```

The number of genes roughly corresponds to the false positive rates. We apply stricter FDR levels to understand if there are any genes for which we can be sure about differential essentiality across cell lines. 
...
```
```{r}
intersect(extracted_gene_categories_K562_best_strict$not_essential_genes_I,extracted_gene_categories_HT29_strict$essential_genes_I)
```
```{r}
